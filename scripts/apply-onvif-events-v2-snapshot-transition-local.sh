...
BACKUP_DIR="$PROJECT_DIR/backups/onvif-events-v2-snapshot-transition-local-$STAMP"

if [[ "$(id -u)" -ne 0 ]]; then
  echo "Run as root: sudo bash scripts/apply-onvif-events-v2-snapshot-transition-local.sh" >&2
  exit 1
fi
...
if (!source.includes('const snapshotStates = new Map<string, string>();')) {
...
if (!source.includes('snapshotTransitions:')) {
  if (source.includes('detailLog: String(process.env.ONVIF_EVENT_DETAIL_LOG')) {
    source = source.replace(
      /    detailLog: String\(process\.env\.ONVIF_EVENT_DETAIL_LOG \|\| 'true'\)\.toLowerCase\(\) !== 'false'\s*\n  };\n}/,
...
const pollBlock = `    const messages = await pullMessages(session.pullPoint, creds.username, creds.password);
...
const detailLogBlock = `      if (config.detailLog) {
...
source = source.replace(
  /      if \(config\.detailLog\) \{\n        const samples = events\.slice\(0, 8\)\.map\(eventDebugSample\);\n        console\.log\('\[onvif-events:v2\] ' \+ camera\.stream_name \+ ' stored events:[\s\S]*?\n      \}/,
  detailLogBlock
);
source = source.replace(
  /      console\.log\('\[onvif-events:v2\] stored events', \{\n        stream_name: camera\.stream_name,\n        events: events\.length,\n        inserted,\n        lastEventAt: session\.lastEventAt\n      \}\);/,
...
if (!source.includes("v145-snapshot-state-transitions") || !source.includes('snapshotTransitions:') || !source.includes('snapshotStateKey') || !source.includes('SnapshotStateChanged')) {
...
echo
echo "ONVIF v2 snapshot transition local patch applied. Backup: $BACKUP_DIR"
