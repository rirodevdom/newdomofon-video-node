...
BACKUP_DIR="$PROJECT_DIR/backups/v126-onvif-events-reconnect-watchdog-$(date +%Y%m%d-%H%M%S)"

echo "===== v126 ONVIF events reconnect watchdog ====="
...
mkdir -p "$BACKUP_DIR$dVR_DIR" 2>/dev/null || true
...
const VERSION = 'v126-onvif-events-reconnect-watchdog';
...
grep -q "v126-onvif-events-reconnect-watchdog" "$EVENTS_JS"
...
Rollback:
  LAST_BACKUP="\$(ls -td /opt/newdomofon-video/backups/v126-onvif-events-reconnect-watchdog-* | head -1)"
