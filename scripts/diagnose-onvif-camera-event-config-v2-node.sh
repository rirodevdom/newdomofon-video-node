...
OUT_DIR="${OUT_DIR:-/tmp/newdomofon-onvif-camera-event-config-v2-$(date +%Y%m%d-%H%M%S)}"
JS_FILE="$OUT_DIR/diagnose-onvif-camera-event-config-v2.js"
...
function envelope(body, user, pass){
  return `<?xml version="1.0" encoding="UTF-8"?><s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:tds="http://www.onvif.org/ver10/device/wsdl" xmlns:trt="http://www.onvif.org/ver10/media/wsdl" xmlns:tan="http://www.onvif.org/ver20/analytics/wsdl" xmlns:tad="http://www.onvif.org/ver10/analytics/wsdl" xmlns:tev="http://www.onvif.org/ver10/events/wsdl" xmlns:tt="http://www.onvif.org/ver10/schema"><s:Header>${wsse(user, pass)}</s:Header><s:Body>${body}</s:Body></s:Envelope>`;
}
...
function topKeys(root){ const keys=new Set(); walk(root,(k,v)=>{ if(/Motion|Rule|Analytics|Topic|XAddr|Profile|Token|Capability|Service|Event/i.test(k)) keys.add(k); }); return Array.from(keys).slice(0,100); }
function profileTokens(root){ const out=[]; for(const p of all(root,'Profiles')) if(p && typeof p==='object') out.push(String(p['@_token'] || p.token || '')); return out.filter(Boolean); }
async function one(cam){
  ...
  console.log(JSON.stringify({stream:cam.stream_name, phase:'device-info', ok:info.ok, status:info.status, manufacturer:first(info.json,['Manufacturer']), model:first(info.json,['Model']), firmware:first(info.json,['FirmwareVersion'])}));
  ...
  const eventX = serviceXaddr(services.json, '/events') || cam.onvif_xaddr;
  const analyticsX = serviceXaddr(services.json, '/analytics') || '';
  const mediaX = serviceXaddr(services.json, '/media') || cam.onvif_xaddr;
  console.log(JSON.stringify({stream:cam.stream_name, phase:'services', ok:services.ok, status:services.status, eventXaddr:eventX, analyticsXaddr:analyticsX, mediaXaddr:mediaX, keys:topKeys(services.json).slice(0,25)}));
  ...
  console.log(JSON.stringify({stream:cam.stream_name, phase:'capabilities', ok:caps.ok, status:caps.status, keys:topKeys(caps.json).slice(0,30)}));
  ...
  console.log(JSON.stringify({stream:cam.stream_name, phase:'event-properties', ok:props.ok, status:props.status, keys:topKeys(props.json).slice(0,50)}));
  ...
  console.log(JSON.stringify({stream:cam.stream_name, phase:'profiles', ok:profiles.ok, status:profiles.status, tokens:profileTokens(profiles.json), keys:topKeys(profiles.json).slice(0,50)}));
  if(analyticsX){
    const rules = await soap(analyticsX, 'http://www.onvif.org/ver20/analytics/wsdl/GetSupportedRules', '<tan:GetSupportedRules/>', c.username, c.password, `${prefix}-supported-rules`);
    console.log(JSON.stringify({stream:cam.stream_name, phase:'supported-rules', ok:rules.ok, status:rules.status, keys:topKeys(rules.json).slice(0,80)}));
    const modules = await soap(analyticsX, 'http://www.onvif.org/ver20/analytics/wsdl/GetSupportedAnalyticsModules', '<tan:GetSupportedAnalyticsModules/>', c.username, c.password, `${prefix}-supported-analytics-modules`);
    console.log(JSON.stringify({stream:cam.stream_name, phase:'supported-analytics-modules', ok:modules.ok, status:modules.status, keys:topKeys(modules.json).slice(0,80)}));
  }
...
echo "grep -RniE 'Manufacturer|Model|Firmware|RuleEngine|Motion|IsMotion|Analytics|CellMotion|Rule|Topic|XAddr|GetSupported|VideoAnalytics|Fault|MustUnderstand' $OUT_DIR | head -300"
